...
void loop(){
  switch(STATE){
    ...
    case 1:
      digitalWrite(25, 1);
      if (touch_data ==1) STATE=2;
      if (newcommand == 1) {
        if((todo_byte >> 6) & 0x01) {
          client = ((uint32_t)client_num[0] << 24) | ((uint32_t)client_num[1] << 16) | ((uint32_t)client_num[2] << 8) | client_num[3];
    ...
    //display.setFullWindow();
          ...
          if (OK==1){
            OK=0;
            client_num[0] = (client >> 24) & 0xFF;  // extract the most significant byte
            client_num[1] = (client >> 16) & 0xFF;  // extract the second byte
            client_num[2] = (client >> 8) & 0xFF;   // extract the third byte
            client_num[3] = client & 0xFF;          // extract the least significant byte
            STATE=3;
          }
        }
        break;
