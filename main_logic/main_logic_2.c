...
uint16_t unitprice = 985, factor=368;
uint32_t converter = 10;
uint32_t litros_set=0, pesos_set=0, litros_act=0, target =0;
...
/*	
	...
	*/
    ...
    while (1) {
		...
		switch (STATE){
				case 0:
					if ((milliseconds-currentMillis)>=5000){	//check error status every 5000ms
						check_status();
						currentMillis=milliseconds;
					}
					i2c_write_blocking(i2c0, 0x5C, &REG_STATUS, 1, true);//check STATUS of control board
					...
					break;
				case 1:
					if ((milliseconds-currentMillis)>=5000){	//check error status every 500ms
						check_status();
						currentMillis=milliseconds;
					}
					i2c_write_blocking(i2c0, 0x5A, &REG_STATUS, 1, true);//check STATUS of display board
					i2c_read_blocking(i2c0, 0x5A, readBuffer, 1, false);
					if (readBuffer[0] == 3){
						i2c_write_blocking(i2c0, 0x5A, &REG_DISPALY_NUMBER, 1, true);
						i2c_read_blocking(i2c0, 0x5A, buffer, 4, false);
						client_number = ((uint32_t)buffer[0] << 24) | ((uint32_t)buffer[1] << 16) | ((uint32_t)buffer[2] << 8) | buffer[3]; //we know how many litros need to load
						STATE=2;
					}
					break;
				case 2:
					//we need to send client number to internet board
					buffer[0] = (client_number >> 24) & 0xFF;  // extract the most significant byte
					buffer[1] = (client_number >> 16) & 0xFF;  // extract the second byte
					buffer[2] = (client_number >> 8) & 0xFF;   // extract the third byte
					buffer[3] = client_number & 0xFF;          // extract the least significant byte
					i2c_write_blocking(i2c0, 0x5E, &REG_CLIENT_NUMBER, 1, true);
					i2c_write_blocking(i2c0, 0x5E, buffer, 4, false);
					i2c_write_blocking(i2c0, 0x5E, &REG_TODO, 1, true);
					i2c_read_blocking(i2c0, 0x5E, buffer, 1, false);
					temp_data = readBuffer[0];
					temp_data |= (1 << 6); //command to find client
					buffer[0] = temp_data;
					i2c_write_blocking(i2c0, 0x5E, &REG_TODO, 1, true);
					i2c_write_blocking(i2c0, 0x5E, buffer, 1, false);
					STATE = 3;
					break;
				case 3:
					//wait flag of data ready
					i2c_write_blocking(i2c0, 0x5E, &REG_STATUS, 1, true);
					i2c_read_blocking(i2c0, 0x5E, buffer, 1, false);
					temp_data = readBuffer[0];
					bit_value = (temp_data >> 3) & 0x01;
					if (bit_value == 1) STATE=5;
					break;
				case 5:
					//read client number
					...
					client_number = ((uint32_t)buffer[0] << 24) | ((uint32_t)buffer[1] << 16) | ((uint32_t)buffer[2] << 8) | buffer[3];
					//read litros
					i2c_write_blocking(i2c0, 0x5E, &REG_LITROS, 1, true);
					i2c_read_blocking(i2c0, 0x5E, buffer, 2, false);
					litros = (buffer[1] << 8) | buffer[0];
					//read price
					i2c_write_blocking(i2c0, 0x5E, &REG_PRICE, 1, true);
					i2c_read_blocking(i2c0, 0x5E, buffer, 2, false);
					unitprice = (buffer[1] << 8) | buffer[0];
					//read factor
					i2c_write_blocking(i2c0, 0x5E, &REG_FACTOR, 1, true);
					i2c_read_blocking(i2c0, 0x5E, buffer, 2, false);
					factor = (buffer[1] << 8) | buffer[0];
					//read litros
					i2c_write_blocking(i2c0, 0x5E, &REG_NOMBRE, 1, true);
					i2c_read_blocking(i2c0, 0x5E, nombre, 42, false);
					//write properly data to display module:
					//client number
					buffer[0] = (client_number >> 24) & 0xFF;  // extract the most significant byte
					buffer[1] = (client_number >> 16) & 0xFF;  // extract the second byte
					buffer[2] = (client_number >> 8) & 0xFF;   // extract the third byte
					...
					i2c_write_blocking(i2c0, 0x5A, buffer, 2, false);
					//how many litros
					...
					i2c_write_blocking(i2c0, 0x5A, buffer, 4, false);
					//send start command to display module
				...
				case 6:
					...
					i2c_read_blocking(i2c0, 0x5C, readBuffer, 1, false);
					temp_data = readBuffer[0];
					bit_value = (temp_data >> 6) & 0x01; //if Valve open
					if (bit_value == 1) STATE=7;
					break;
				case 7:
					if ((milliseconds-currentMillis)>=1000){	//check actual number of liters
						//read counter number
					...
					i2c_read_blocking(i2c0, 0x5C, readBuffer, 1, false);
					temp_data = readBuffer[0];
					bit_value = (temp_data >> 6) & 0x01; //if valve close
					if (bit_value == 0){
						 //read counter number
						i2c_write_blocking(i2c0, 0x5C, &REG_COUNTER, 1, true);
						i2c_read_blocking(i2c0, 0x5C, buffer, 4, false);
				...
				case 8:
					...
					i2c_write_blocking(i2c0, 0x5C, &REG_TODO, 1, true);
					i2c_read_blocking(i2c0, 0x5C, buffer, 1, false);
					temp_data = readBuffer[0];
					temp_data &= ~((1 << 6));//stop process
					temp_data |= (1 << 5); //start process
					buffer[0] = temp_data;
					i2c_write_blocking(i2c0, 0x5C, &REG_TODO, 1, true);
					i2c_write_blocking(i2c0, 0x5C, buffer, 1, false);
					...
					i2c_write_blocking(i2c0, 0x5A, &REG_SET_LITROS, 1, true);
					i2c_read_blocking(i2c0, 0x5A, buffer, 4, false);
					litros_print = ((uint32_t)buffer[0] << 24) | ((uint32_t)buffer[1] << 16) | ((uint32_t)buffer[2] << 8) | buffer[3];
					
					i2c_write_blocking(i2c0, 0x5A, &REG_SET_PESOS, 1, true);
					i2c_read_blocking(i2c0, 0x5A, buffer, 4, false);
					pesos_print = ((uint32_t)buffer[0] << 24) | ((uint32_t)buffer[1] << 16) | ((uint32_t)buffer[2] << 8) | buffer[3];
					
					//need to add log data sending here
					
					sleep_ms(60000); //1min pause
